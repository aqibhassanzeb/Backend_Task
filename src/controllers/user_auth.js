...
export const userLogin = async (req, res) => {
  ...
  bycrypt.compare(password, user_find.password).then((doMatch) => {
    if (doMatch) {
...
export const userLoginwithGmail = async (req, res) => {
  ...
  try {
    let user_find = await User.findOne({ email });
    if (user_find) {
      if (user_find.status !== status)
        return res
          .status(400)
          .json({ error: `email already register for ${user_find.status}` });
      const token = jwt.sign({ _id: user_find._id }, process.env.JWT_SECRET);
      return res.json({ message: "Successfull Login", token, user: user_find });
    }
